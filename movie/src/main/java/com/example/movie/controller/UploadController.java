...
public class UploadController {
    ...
    // application.properties에 작성한 값 불러오기(하드코딩하면 안되는 값들 대상)
    ...
    public ResponseEntity<List<UploadResultDTO>> postUpload(MultipartFile[] uploadFiles) {
        ...
        for (MultipartFile uploadFile : uploadFiles) {
            ...
            // 자바의 유효 아이디(일반적으로도 많이 사용함.)/ universally unique identifier (UUID)약자
            // 고유의 식별자 생성(중복안됨)
            String uuid = UUID.randomUUID().toString();

            // upload/2025/05/15/235a046c-7ce1-43b3-b9e5-adfa9db3decb_파일명.jpg
            String saveName = uploadPath + File.separator + saveFolderPath + File.separator + uuid + "_" + oriName;
            Path savePath = Paths.get(saveName);

            try {
                // 경로에 실제로 옮겨 특정 폴더(여기선 서버폴더)에 저장
                uploadFile.transferTo(savePath);

                // 썸네일 저장
                String thumbnailSavedName = uploadPath + File.separator + saveFolderPath + File.separator + "s_"
                        + uuid + "_" + oriName;
    ...
    public ResponseEntity<byte[]> getFile(String fileName, String size) {
        ...
        try {
            ...
            // Content-Type : "application/json" "image/jpg" ==> MIME 확인
    ...
    private String makeFolder() {
        ...
        // LocalDate.now() : 오늘날짜 2025-5-25 => format : 2025/05/15
        String dateStr = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy/MM/dd"));

        // File.separator : 운영체제에 맞는 파일구분기호(/ 또는 \)
        String folderPath = dateStr.replace("/", File.separator);

        File uploadPathFolder = new File(uploadPath, folderPath);

        if (!uploadPathFolder.exists()) // "!" 존재하지 않으면,
            uploadPathFolder.mkdirs(); // 디렉토리 생성하라. (make directory) 약자
        return folderPath;
